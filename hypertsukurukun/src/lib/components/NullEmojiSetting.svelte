<script lang="ts">
	import { nullEmojiStore, selectedEmojiStore, selectEmoji, deselectEmoji } from "$lib/stores";
	import { selectEmoji as paletteSelectEmoji } from "$lib/stores/palette";
...
<div class="null-emoji-setting">
	...
	{#if isLoaded}
		<div class="current-null">
			{#if nullConfig.type === "custom" && nullConfig.emoji}
				<img src={nullConfig.emoji.url} alt={nullConfig.emoji.shortcode} class="null-preview" />
			{/if}
	...
	<!-- パレットから選択ボタン -->
	<button class="select-btn" onclick={startPaletteSelection}>パレットから選択</button>
